...
exports.createProduct = (req, res) => {
    ...
    form.parse(req, (err, fields, file) => {
        ...
        const {product_name, product_price, description, adderess, stock, category_type } = fields;

        if(!product_name || !product_price || !description || !adderess || !stock || !category_type) {
            return res.status(400).json({
...
exports.getAllProductsById = (req, res) => {
    Product.find()
    .populate("product", "_id product_name")
    .exec((err, products)=> {
        if(err) {
            return res.status(400).json({
                error : "No products"
